...
if [ "$LOG" = "true" ]; then
	echo ">>> Run cmd 3 ReleaseUpdate.sh prepare : test if BUILD_TYPE='stable' BUILD_TYPE is set to $BUILD_TYPE"
	echo ">>> Run cmd 3 ReleaseUpdate.sh prepare : test if BUILD_VERSION is empty BUILD_VERSION is set to $BUILD_VERSION"
fi
