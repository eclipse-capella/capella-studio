...
PRODUCT_PRJ_NAME="org.polarsys.capella.studio.releng.product"
...
if [ "$LOG" = "true" ]; then
	echo ">>> Run cmd 1 ReleaseProducts.sh start : . $BASEDIR/release-parameters.sh $PRODUCT_NAME $COMPONENT_NAME $BUILD_TYPE"
fi
# Extract global parameter (ie VERSION)
...
TARGET_MANIFEST_FOLDER="releng/plugins/$PRODUCT_PRJ_NAME/target"
...
if [ "$LOG" = "true" ]; then
	echo ">>> Run cmd 4 ReleaseProducts.sh prepare : test if BUILD_TYPE='stable' BUILD_TYPE is set to $BUILD_TYPE"
fi
