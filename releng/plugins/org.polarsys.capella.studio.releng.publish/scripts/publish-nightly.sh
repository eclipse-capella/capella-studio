...
if [ "$LAUNCH_CAPELLA" = "true" ]; then
	...
	echo "  Update nightly url to listen : $URL_PUBLISH_PREFIX/$BUILD_TYPE/$COMPONENT_NAME/latest/trigger/launch_cappella.txt" >>  $MANIFEST_FILE
	mkdir -p $TARGET_ROOT/$COMPONENT_NAME/latest/trigger
	cp -dR $MANIFEST_FILE $TARGET_ROOT/$COMPONENT_NAME/latest/trigger/launch_cappella.txt
else
